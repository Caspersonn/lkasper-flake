{ ... }: {
  flake.modules.homeManager.lucak-aws = { lib, config, unstable, ... }:
    let
      ...
      userPath = /. + "${config.home.homeDirectory}/.aws/managed_service_accounts.json";
      fallbackPath = /home/casper/.aws/managed_service_accounts.json;
      technative_profiles = builtins.path {
        path = if builtins.pathExists userPath then userPath else fallbackPath;
      ...
      alternative_regions = {
        ...
        "562622988252" = "us-east-1"; # Mustad management
      };
      alternative_names = {
      ...
      normalize_group = group:
        __concatStringsSep "_"
        (builtins.filter (x: builtins.typeOf x == "string")
          (__split " " (lib.strings.toLower group)));

      shortname_group = account:
        let
          shortname_temp = if builtins.hasAttr groupnorm groups
          && builtins.hasAttr "shortname" groups.${groupnorm} then
      ...
      show_account = account:
        ...
        in if (builtins.hasAttr groupnorm groups
          && builtins.hasAttr "ignore" groups.${groupnorm}
          && groups.${groupnorm}.ignore == true)
        || (builtins.hasAttr accountnorm account_names
          && builtins.hasAttr "ignore" account_names.${accountnorm}
          && account_names.${accountnorm}.ignore == true) then
          false
        else
          true;

      tn_profile = { account_id, group }:
        let groupnorm = normalize_group group;
        in {
          inherit group;
          source_profile = "technative";
          role_arn =
            "arn:aws:iam::${account_id}:role/landing_zone_devops_administrator";
          region = if builtins.hasAttr account_id alternative_regions then
            alternative_regions."${account_id}"
          else
            "eu-central-1";
          color = if builtins.hasAttr groupnorm groups
          && builtins.hasAttr "color" groups.${groupnorm} then
            groups.${groupnorm}.color
          else
